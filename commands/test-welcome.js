...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Previsualiza el embed de bienvenida (solo para administradores).')
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
    category: 'manager',
    description: 'Previsualiza el embed de bienvenida (solo para administradores).',
    usage: '!!test-welcome',
    async execute(message, args) {
        ...
        if (!config || !config.channelId) {
            return message.reply('La configuración de bienvenida no ha sido establecida.');
        }

        const { embed, content } = createWelcomeFarewellEmbed(message.member, config);
        await message.reply({ content: content || 'Esta es una vista previa del embed de bienvenida:', embeds: [embed] });
    },
    async executeSlash(interaction) {
        const config = getWelcomeConfig(interaction.guild.id);
        if (!config || !config.channelId) {
            return interaction.reply({ content: 'La configuración de bienvenida no ha sido establecida.', flags: MessageFlags.Ephemeral });
        }
